...
//PET INICIO
    ...
    public void gainPetYears7(int itemId) {
        client.Character chr = getPlayer();

        if (chr.getInventory(client.inventory.InventoryType.CASH).getNextFreeSlot() == -1) {
            chr.dropMessage(5, "Tu inventario de Cash está lleno.");
            return;
        }

        // Pet vivo (lvl 1, closeness 0, fullness 100)
        int petId = client.inventory.Pet.createPet(itemId, (byte) 1, 0, 100);
        if (petId <= 0) {
            chr.dropMessage(5, "Error al crear la mascota.");
            return;
        }

        // Expiración exacta +7 años (ms)
        java.util.Calendar cal = java.util.Calendar.getInstance();
        cal.add(java.util.Calendar.YEAR, 7);
        long expirationMs = cal.getTimeInMillis();
        ...
        chr.dropMessage(5, "¡Mascota entregada (vigencia 7 años, vida llena)!");
    }
