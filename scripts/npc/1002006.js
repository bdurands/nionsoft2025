...
// LEVEL 51
  ...
  { pid:403, name:"Grizly",         min:51, max:60,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:2000000, exp:200000, item:4310000, qty:1,  mobs:[5120500]
  },
...
// LEVEL 61
  ...
  { pid:503, name:"Jester Scarlion",min:61,max:70,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:2000000, exp:400000, item:4310000, qty:1,  mobs:[9420535]
  },
...
// LEVEL 71
  ...
  { pid:602, name:"Death Teddy",    min:71, max:90,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:2500000, exp:600000, item:4310000, qty:1,  mobs:[7130010]
  },
...
// LEVEL 91
  ...
  { pid:703, name:"Green Cornian",  min:91, max:110,
    targetByJob: { warrior:700, thief:700, pirate:700, bowman:1000, mage:1500, default:800 },
    mesos:3000000, exp:1500000, item:4310000, qty:1,  mobs:[8150200]
  },
...
// LEVEL 111
  ...
  { pid:802, name:"Dark Wyvern",    min:111,max:140,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:4000000, exp:4000000, item:4310000, qty:1,  mobs:[8150302]
  },
...
// LEVEL 141
  ...
  { pid:902, name:"Chief Memory Guardian", min:141,max:170,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:5000000, exp:8000000, item:4310000, qty:1, mobs:[8200004]
  },
...
// LEVEL 171
  ...
  { pid:1002, name:"Chief Oblivion Guardian", min:171,max:200,
    targetByJob: { warrior:1200, thief:1200, pirate:1200, bowman:1200, mage:1500, default:1200 },
    mesos:5000000, exp:20000000, item:4310000, qty:1, mobs:[8200012]
  }
];
...
function action(mode, type, selection){
  ...
  if(status==0){
    ...
    var header="#eDaily Kill Quests#n (level: #b"+lv+"#k)\r\n"
             +"Today's progress: #b"+killCount+"#k / #b"+targetCount+"#k\r\n"
             +"Active quest: "+(targetCount>0?("#b"+activeQuestName+"#k"):"#r(none)#k")+"\r\n"
             +"Completed today: #b"+getCompletedQuestNames(completedToday)+"#k\r\n\r\n";
    var body="";
    ...
    if(selection==1){
      ...
      var msg="Active quest: "+(activePreset?("#b"+activePreset.name+"#k"):"#r(none)#k")+"\r\n"
            +"Progress: #b"+(progress?progress.killCount:0)+"#k / #b"+(progress?progress.targetCount:0)+"#k\r\n"
            +"Valid mobs: #b"+(activePreset?mobNames(activePreset.mobs):"(none)")+"#k\r\n"
            +"Completed today: #b"+completedList+"#k\r\n";

      if (activePreset) {
        msg += "Reward: #b"+activePreset.mesos+" mesos#k"
      ...
      if (progress && progress.presetId === p.pid && progress.targetCount > 0 && !progress.claimed) {
        pushHelper();
        cm.sendOk("This preset is already active.\r\nProgress: #b"+progress.killCount+"#k / #b"+progress.targetCount+"#k.");
        cm.dispose();
        return;
      }

      if (LOCK_PRESET_AFTER_PROGRESS && progress && progress.killCount > 0 && !progress.claimed && progress.presetId !== p.pid) {
        cm.sendOk("You already have progress today on another quest. Claim it or wait for the cooldown to finish before changing.");
        cm.dispose();
        return;
      }
